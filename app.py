"""
AgroSense — Backend complet
Rôles : admin (1 seul), agriculteur, invité
"""
...
def init_db():
    ...
    # Toujours mettre à jour le hash admin au démarrage (évite les conflits de volume)
    ADMIN_PWD = "admin2025"
    ...
    if not existing_admin:
        ...
        # Resynchroniser le hash au cas où le volume aurait un hash différent
...
def _seed_demo():
    for ag_username, parcelles_names in [("diallo",["Parcelle Nord","Parcelle Sud"]),("sow",["Champ Est"])]:
        ...
        for pname in parcelles_names:
            ...
            for tc in [
                    {"type":"température","unite":"°C"},
                    {"type":"humidité","unite":"%"},
                    {"type":"pH sol","unite":"pH"},
                    {"type":"pluviométrie","unite":"mm"},
                    {"type":"vitesse vent","unite":"km/h"},
                    {"type":"luminosité","unite":"lux"},
                    {"type":"pression","unite":"hPa"},
                ]:
    ...
    print("🌱 Données démo générées.")

def _gen(t, ts):
    h = ts.hour
    if t == "température":
        return round(24+8*(1-abs(h-14)/14)+random.gauss(0,1.5), 2)
    if t == "humidité":
        v = 55-20*(1-abs(h-14)/14)+random.gauss(0,5)
        if random.random()<0.10: v=random.uniform(15,28)
        return round(max(10,min(100,v)), 2)
    if t == "pH sol":
        return round(random.gauss(6.5,0.3), 2)
    if t == "pluviométrie":
        # Pluie surtout la nuit/matin
        base = 2.5 if h < 8 or h > 20 else 0.2
        v = max(0, base + random.gauss(0, 0.8))
        if random.random() < 0.05: v = random.uniform(8, 25)  # averse
        return round(v, 2)
    if t == "vitesse vent":
        return round(max(0, random.gauss(12, 5)), 2)
    if t == "luminosité":
        # Soleil le jour, 0 la nuit
        if h < 6 or h > 20: return 0
        lux = 50000 * (1 - abs(h-13)/8) + random.gauss(0, 5000)
        return round(max(0, lux), 0)
    if t == "pression":
        return round(random.gauss(1013, 3), 1)
    return 0.0
...
def insertion_continue():
    ...
    while True:
        ...
        if caps:
            ...
            print(f"[{now.strftime('%H:%M:%S')}] ✅ {len(caps)} mesures insérées.")
...
def auth_login():
    ...
    if user["statut"]=="en_attente": return jsonify({"error":"Compte en attente de validation par l'administrateur"}),403
    if user["statut"]=="rejeté":     return jsonify({"error":"Votre inscription a été refusée"}),403
...
def auth_register():
    ...
    if role=="admin": return jsonify({"error":"Impossible de créer un admin via l'inscription"}),403
...
def get_parcelles():
    u = session["user"]
    ag_id = request.args.get("agriculteur_id")
...
def add_capteur():
    d   = request.get_json() or {}
    pid = d.get("parcelle_id","")
    typ = d.get("type","")
    if not pid or not typ: return jsonify({"error":"parcelle_id et type requis"}),400
    u  = session["user"]
    p  = parcelles_col.find_one({"_id":ObjectId(pid),"agriculteur_id":u["id"]})
...
@login_required
def get_alertes():
    sh = float(request.args.get("seuil_hum",30))
    st = float(request.args.get("seuil_temp",38))
    f  = _filtre_base(24)
    alertes=[]
...
def admin_ag_stats(ag_id):
    """Stats complètes d'un agriculteur pour l'admin."""
    now = datetime.utcnow()
    ...
    if not ag:
        return jsonify({"error": "Agriculteur introuvable"}), 404
    ...
    nb_capteurs   = capteurs_col.count_documents({"agriculteur_id": ag_id, "actif": True})
    nb_mesures_24h= mesures_col.count_documents({"agriculteur_id": ag_id, "timestamp": {"$gte": depuis_24h}})
    nb_mesures_7j = mesures_col.count_documents({"agriculteur_id": ag_id, "timestamp": {"$gte": depuis_7j}})
    nb_alertes    = mesures_col.count_documents({"agriculteur_id": ag_id, "timestamp": {"$gte": depuis_24h},
        "$or":[{"type":"humidité","valeur":{"$lt":30}},{"type":"température","valeur":{"$gt":38}}]})

    # Dernières mesures par type
    derniers = {}
    for t in ["température","humidité","pH sol","vitesse vent","pluviométrie","luminosité","pression"]:
        m = mesures_col.find_one({"agriculteur_id":ag_id,"type":t,"timestamp":{"$gte":depuis_24h}},
            {"_id":0}, sort=[("timestamp",DESCENDING)])
        if m: m["timestamp"] = m["timestamp"].isoformat(); derniers[t] = m

    # Moyennes par parcelle (température)
    pipeline_temp = [
        {"$match":{"agriculteur_id":ag_id,"type":"température","timestamp":{"$gte":depuis_24h}}},
        {"$group":{"_id":"$parcelle_nom","moyenne":{"$avg":"$valeur"},"min":{"$min":"$valeur"},"max":{"$max":"$valeur"}}},
        {"$sort":{"_id":1}}
    ]
    stats_temp = list(mesures_col.aggregate(pipeline_temp))
    for r in stats_temp:
        r["parcelle"] = r.pop("_id")
        for k in ["moyenne","min","max"]: r[k] = round(r[k],2)
    ...
    return jsonify({
        ...
        "nb_mesures_24h": nb_mesures_24h,
        "nb_mesures_7j": nb_mesures_7j,
...
def admin_ag_mesures(ag_id):
    """Mesures d'un agriculteur pour l'admin."""
    heures = int(request.args.get("heures", 24))
    limit  = int(request.args.get("limit", 200))
    typ    = request.args.get("type")
...
@app.route("/api/admin/agriculteur/<ag_id>/alertes")
@role_required("admin")
def admin_ag_alertes(ag_id):
    """Alertes d'un agriculteur pour l'admin."""
    depuis = datetime.utcnow() - timedelta(hours=24)
    alertes = []
    base = {"agriculteur_id": ag_id, "timestamp": {"$gte": depuis}}
    for m in mesures_col.find({**base,"type":"humidité","valeur":{"$lt":30}},{"_id":0}).sort("timestamp",DESCENDING).limit(50):
        m["timestamp"]=m["timestamp"].isoformat(); m["niveau"]="critique" if m["valeur"]<20 else "avertissement"; m["message"]=f"Humidité : {m['valeur']}%"; alertes.append(m)
    for m in mesures_col.find({**base,"type":"température","valeur":{"$gt":38}},{"_id":0}).sort("timestamp",DESCENDING).limit(50):
        m["timestamp"]=m["timestamp"].isoformat(); m["niveau"]="critique"; m["message"]=f"Température : {m['valeur']}°C"; alertes.append(m)
    for m in mesures_col.find({**base,"type":"pH sol","$or":[{"valeur":{"$lt":5.5}},{"valeur":{"$gt":7.5}}]},{"_id":0}).sort("timestamp",DESCENDING).limit(50):
        m["timestamp"]=m["timestamp"].isoformat(); m["niveau"]="avertissement"; m["message"]=f"pH : {m['valeur']}"; alertes.append(m)
    alertes.sort(key=lambda x:x["timestamp"],reverse=True)
    return jsonify(alertes[:100])
...
def admin_global_alertes():
    """Toutes les alertes de tous les agriculteurs."""
    depuis = datetime.utcnow() - timedelta(hours=24)
    alertes = []
...
def admin_rapport_global():
    """Rapport global : stats par agriculteur."""
    ags = list(users_col.find({"role":"agriculteur","statut":"actif"}, {"_id":1,"nom":1,"username":1}))
    rapport = []
    ...
    for ag in ags:
        ...
        rapport.append({
            ...
            "nb_capteurs": capteurs_col.count_documents({"agriculteur_id":ag_id,"actif":True}),
            "nb_mesures":  mesures_col.count_documents({"agriculteur_id":ag_id,"timestamp":{"$gte":depuis_24h}}),
            "nb_alertes":  mesures_col.count_documents({"agriculteur_id":ag_id,"timestamp":{"$gte":depuis_24h},"$or":[{"type":"humidité","valeur":{"$lt":30}},{"type":"température","valeur":{"$gt":38}}]}),
        })
...
if __name__ == "__main__":
    ...
    print("🌱 AgroSense démarré")
    print("👑 Admin  → http://localhost:5000/admin/login  (admin / admin2025)")
    print("🌾 Users  → http://localhost:5000/login")
